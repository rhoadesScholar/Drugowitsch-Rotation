...
StaticWorld(A::Array{Float64,2}, C::Array{Float64,2}, muPrior::Array{Float64,1}, endT::Float64, dt::Float64) =
    StaticWorld(A, C, muPrior, endT, dt, Integer(ceil(endT/dt)))
StaticWorld(A::Array{Float64,2}, C::Array{Float64,2}, muPrior::Array{Float64,1}) =
    StaticWorld(A, C, muPrior, 500., .5, 1000)
StaticWorld(A::Array{Float64,2}, C::Array{Float64,2}, muPrior::Array{Float64,1}, endT::Float64, dt::Float64, endI::Int) =
...
function runSim(kworld::FullWorld)
    ...
    Mus = Array{Array{Float64,1},1}(undef, kworld.static.endI+1)
    LLike = Array{Float64,1}(undef, kworld.static.endI+1)
    LLike[1] = 0;
    ...
    for i = 2:kworld.static.endI+1
        ...
        LLike[i] = logaddexp(LLike[i-1], stableLogPDF(kworld.static.C*Mus[i], kworld.static.C*kworld.Vars[i]*kworld.static.C', Ys[i]))#logpdf(MvNormal(kworld.static.C*Mus[i], kworld.static.C*kworld.Vars[i]*kworld.static.C'), Ys[i])
    end
    return vcat(hcat(se.(Mus-Zs)...), LLike')
end

function stableLogPDF(Mu, Cov, Y)
    d = length(Mu)
    pdf = log(((2*pi)^(-d/2))*(det(Cov)^(-1/2))*exp(-((Y-Mu)'*inv(Cov)*(Y-Mu))/2))
    # logsumexp(!!!!!!!!!!!!!!!!!!!!!!!!!!!)
    return pdf
end

function se(M::Array{Float64,1})#FIX
    return M.^2#SQRT CHANGES METRIC FROM VARIANCE (JAN MEETING 2/3/20), note:summing across dimensions sums together variances (changed to averaging)
end
